# SK Epic 1.0 -- Marl_Kastane in Tox Forest
...
if(quest::is_the_ruins_of_kunark_enabled()){
	sub EVENT_SAY {
		...
		if($skepic_notetomarl == 1) {
			...
			if ($text=~/prophecy/i) {
				...
				quest::summonitem(14375); #Seal of Kastane
			}
		}
	}
	...
	sub EVENT_ITEM {
		...
		if ($itemcount{14376} == 1) {
			quest::emote("Marl Kastane reads the note, eyes widening in panic. He raises his eyes to you and says,'This is grave news, friend. If the prophecy is allowed to be fulfilled, our strength will vanish into obscurity. Goodness and purity will spread like a plague across the land and the servants of Hate will be powerless to stop it!'");
			quest::ding();
			quest::faction(342, 10); #TrueSpirit
			quest::setglobal("skepic_notetomarl",1,0,"F");
		}
		#Blood of Kyrenna
		elsif (($skepic_notetomarl == 1) && ($itemcount{14381} == 1)) {
		...
		#Glohnor Wrappings
		elsif (($skepic_notetomarl == 1) && ($itemcount{14379} == 1)) {
			...
			quest::delglobal("skepic_notetomarl");
		} else {
...
# END of FILE Zone:Tox Forest ID:???? Marl Kastane