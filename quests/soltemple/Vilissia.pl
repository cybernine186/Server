...
# Quest for vulcanized platinum bar

sub EVENT_SAY {
  if($text=~/hail/i) {
    quest::say("I am Vilissia, chosen of Solusek Ro.  I am a practitioner in the Wizardly arts, and keeper of lore for the [Runescale Cloak], [Tishan's Kilt] and the [Acumen Mask].");
  }
  ...
  if($text=~/tishans kilt/i) {
    quest::say("Tishan was once counted among the mightiest of wizards – and his items are items of power.  If you are interested in acquiring Tishan's Kilt, you must take a Shadowed Ball from our mortal enemies, the Shadowed Men, and bring it to Trankia in the Everfrost Mountains.");
  }
  if($text=~/acumen mask/i) {
    quest::say("The Acumen Mask, as its name suggests, confers to its wearer certain powers of insight and knowledge. Are you [interested in obtaining a mask]?");
  }
  if($text=~/interested in obtaining a mask/i) {
