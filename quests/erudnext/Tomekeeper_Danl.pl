sub EVENT_SAY {
 ...
 if(quest::is_omens_of_war_enabled()){ 
	 ...
	 if($text=~/book/i){ 
	  # Monk Epic 1.5
	  quest::say("Well I shouldnt say book. It was really a diary of sorts. Kaiaren wrote a diary and it contained all of his disciplines. There may also be a way to reverse the effects of Vorashs attacks located within it. Unfortunately, Vorix would also be the only one that would know the diarys location. Go now and try to locate Vortix and ask him where Kaiarens [diary] may be. Who referred you to me in the first place? Maybe he knows more than he appears to. I remember that Vortix will only address those that call to him by name.");
	 }
 }
}
...
sub EVENT_ITEM {
 ...
 if(quest::is_the_ruins_of_kunark_enabled() && plugin::check_handin(\%itemcount, 18195 => 1)){ 
  # Monk Epic 1.0
 ...
 elsif(quest::is_omens_of_war_enabled() && plugin::check_handin(\%itemcount, 48132 => 1)){ 
  # Monk Epic 1.5
