sub EVENT_SIGNAL {
	...
	#:: Match a signal '4' sent from /fearplane/firewall.pl
	elsif ($signal == 4) {
		my $getmobbynpctype = $entity_list->GetMobByNpcTypeID(2000661);
		my $firewallentree = $getmobbynpctype->GetTarget();
		$npc->AddToHateList($firewallentree,1)
	}
}
