sub EVENT_SAY {
	if ($text=~/hail/i && !defined $qglobals{soulfire}) {
	...
	elsif($text=~/hail/i && defined $qglobals{soulfire}){
	quest::say("You already wield the SoulFire $name!");
 } 
}
...
sub EVENT_ITEM {
	...
	#:: Match a 18937 - Note, a 13947 - Brilliant Sword of Faith, a 18828 - Testimony, and a 12197 - Glowing Sword Hilt
		...
		if (!defined $qglobals{soulfire} && $faction <= 2) {
			...
			quest::setglobal(soulfire, 1, 0, "F");
